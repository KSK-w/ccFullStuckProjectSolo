...
import ListRegisteredPokemon from "./ListRegisteredPokemon";
...
export default function Home() {
  ...
  //IDを採番する関数。削除された番号は無視して、最大の値を返す
  function getMaxID(pokemons) {
    if (!pokemons || pokemons.length === 0) {
      return 0; // リストが空の場合、0 を返す
    }

    // 配列内のID最大値を取得
    return Math.max(...pokemons.map((obj) => obj.ID));
  }

  //現在画面に表示されているポケモンを情報として追加
  async function registerPokemon(pokemon) {
    //IDの採番
    pokemon.ID = getMaxID(registeredPokemons) + 1;
    try {
  ...
  async function deletePokemon(pokemon) {
    try {
      ...
      //フロント側のstateからも削除
      setRegisteredPokemons((currentRegisteredPokemons) =>
        currentRegisteredPokemons.filter((p) => p.ID !== pokemon.ID)
      );
      returnHome();
    } catch (error) {
      console.error("登録エラー:", error);
